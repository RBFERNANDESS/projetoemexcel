...
# Função para calcular o valor de custo
def calcular_valor_custo(valor, quantidade, frete, imposto1, imposto2, imposto3):
    return valor / quantidade + (frete/quantidade) + (valor * (imposto1/100)) + (valor * (imposto2/100)) + (valor * (imposto3/100))
...
def cadastrar_produto():
    ...
    # Calcula o valor de custo
    valor_custo = calcular_valor_custo(valor, quantidade, frete, imposto1, imposto2, imposto3)
    
    # Calcula o valor de venda
    valor_venda = valor_custo * 1.2  # Pode ajustar a margem de lucro conforme necessário
    
    # Adiciona o produto ao arquivo Excel
...
def editar_produto():
    ...
    for row_idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
        if row[0] == nome_produto:
            ...
            # Obtendo a letra da coluna com base na opção
            coluna = ""
            if opcao == "valor":
                coluna = "B"
            ...
            # Atualizando o valor na célula específica
            sheet[f"{coluna}{row_idx}"] = float(novo_valor)

            # Recalcula o valor de custo e valor de venda após a edição
            sheet[f"H{row_idx}"] = calcular_valor_custo(sheet[f"B{row_idx}"].value, sheet[f"C{row_idx}"].value,
                                                         sheet[f"D{row_idx}"].value, sheet[f"E{row_idx}"].value,
                                                         sheet[f"F{row_idx}"].value, sheet[f"G{row_idx}"].value)
            sheet[f"I{row_idx}"] = sheet[f"H{row_idx}"].value * 1.2  # Pode ajustar a margem de lucro conforme necessário
